...
The $\mathcal{I}_{\text{prop}}$ scaling procedure is illustrated in Figure~\ref{fig:propagator_scaling} using the ATLAS $\met+\gamma$ analysis in 36 \ifb of data~\cite{}. Contours are extracted from the grid of individual signal points using the same interpolation and smoothing techniques as the HistFitter package in order to most cleanly reproduce the published results~\cite{histfitter}.

% Introduce that we need full cross section to go between models.
However, a ratio based on $\mathcal{I}_{\text{prop}}$ when the target scenario includes a different mediator model than the source scenario. [Elaborate]
